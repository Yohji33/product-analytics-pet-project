# Итоги анализа

## Обзор данных

Проект использует синтетические e-commerce данные, сгенерированные для портфолио и практики аналитики.

Текущий датасет:

- пользователи: 500
- сессии: 2 225
- оплаченные заказы: 425
- события: 6 752
- выручка: 221 082.40
- средний чек: 520.19
- конверсия из сессии в заказ: 19.10%
- повторные покупатели: 100
- LTV на пользователя: 442.16
- LTV на покупателя: 762.35

## Динамика по месяцам

Самый сильный месяц в датасете - декабрь 2025:

- выручка: 23 873.00
- оплаченные заказы: 48
- конверсия из сессии в заказ: 24.49%

Июль 2025 также показывает высокую активность:

- активные пользователи: 110
- сессии: 205
- оплаченные заказы: 46

Март 2026 нужно интерпретировать осторожно, потому что это неполный месяц в сгенерированных данных.

## Анализ воронки

Шаги воронки:

1. `view_item`
2. `add_to_cart`
3. `purchase`

Результаты:

- пользователи, просмотревшие товар: 500
- пользователи, добавившие товар в корзину: 385
- пользователи, совершившие покупку: 290
- конверсия из просмотра в корзину: 77.00%
- конверсия из корзины в покупку: 75.32%
- конверсия из просмотра в покупку: 58.00%

Основная интерпретация:

Самая большая потеря пользователей происходит между просмотром товара и добавлением в корзину.
Это может указывать на проблемы с карточками товаров, прозрачностью цены, описанием, отзывами или заметностью CTA-кнопки.

## Анализ источников трафика

Лучший источник по конверсии из просмотра в покупку:

- telegram: 32.16%

Самый слабый источник по конверсии из просмотра в покупку:

- google: 23.36%

Интерпретация:

Telegram показывает самую высокую покупательскую конверсию и может приводить более теплую аудиторию.
Но решение о перераспределении бюджета нужно принимать с учетом объема трафика, стоимости привлечения, маржи и retention.

## Анализ каналов привлечения

Лучшие каналы по выручке:

- referral: 51 970.96
- social: 51 335.17
- email: 45 207.53

Лучший канал по конверсии пользователя в покупателя:

- email: 62.37%

Интерпретация:

Referral и social дают наибольшую выручку, а email показывает лучшую конверсию пользователя в покупателя.
Это может означать, что email-аудитория более лояльна или лучше прогрета перед покупкой.

## Экономика каналов привлечения

После добавления маркетинговых расходов стало видно не только то, какие каналы дают выручку, но и то, насколько эффективно они окупаются.

Общие показатели:

- маркетинговые расходы: 65 500.00
- прибыль после маркетинга: 155 582.40
- общий ROMI: 237.53%
- окупаемость маркетинга: 3.38x

Лучший канал по ROMI:

- organic: 623.41%

Самый дорогой канал по CAC:

- ads: 429.82

Интерпретация:

Organic показывает лучшую окупаемость из-за низких расходов на привлечение.
Ads приносит выручку и заказы, но имеет самый высокий CAC, поэтому рекламный бюджет нужно оптимизировать: проверять кампании, сегменты, креативы и качество посадочных страниц.

## Повторные покупки и LTV

Ключевые показатели:

- покупатели: 290
- повторные покупатели: 100
- доля повторных покупателей: 34.48%
- выручка повторных покупателей: 127 706.04
- доля выручки повторных покупателей: 57.76%
- LTV на пользователя: 442.16
- LTV на покупателя: 762.35

Лучшие каналы по LTV/CAC:

- organic: 7.23x
- email: 5.32x
- referral: 4.72x

Интерпретация:

Повторные покупатели дают больше половины выручки.
Это значит, что для роста важны не только новые пользователи, но и удержание: email-цепочки, персональные предложения, рекомендации и реактивация.

## A/B-тест CTA-кнопки

Гипотеза:

Новая CTA-кнопка на карточке товара увеличит конверсию.

Результаты:

- control: 18.00%
- test: 26.00%
- абсолютный uplift: +8.00 п.п.
- относительный uplift: +44.44%
- p-value: 0.031

Интерпретация:

Тестовый вариант статистически значимо лучше control на уровне 5%.
Рекомендация: раскатить test-вариант и продолжить мониторинг конверсии, среднего чека и выручки на пользователя.

## Анализ устройств

Лучший тип устройства по выручке:

- desktop: 97 783.89

Лучший тип устройства по конверсии из сессии в заказ:

- desktop: 21.12%

Самый слабый тип устройства по конверсии:

- mobile: 15.22%

Интерпретация:

Desktop-пользователи конвертируются лучше, чем mobile-пользователи.
Мобильную воронку стоит изучить глубже: UX checkout, скорость загрузки, платежный сценарий и структура карточки товара могут влиять на конверсию.

## Анализ категорий

Лучшие категории по выручке:

- Electronics: 80 141.63
- Home: 67 450.40
- Sports: 48 133.28
- Beauty: 32 551.16

Интерпретация:

Electronics и Home являются основными драйверами выручки.
Beauty показывает самую низкую выручку при сопоставимом количестве проданных товаров, что может объясняться более низкой средней ценой товара.

## Анализ товаров

Лучшие товары по выручке:

- Orion Headphones
- Atlas Laptop
- Pulse Chair
- Aero Storage Box
- Aero Phone

Интерпретация:

Топовые товары сосредоточены в основном в категориях Electronics и Home.
Эти категории стоит приоритизировать в мониторинге дашборда и дальнейшем бизнес-анализе.

## Финальные рекомендации

- оптимизировать `ads`, потому что канал имеет самый высокий CAC
- масштабировать `organic`, `email` и `referral`, потому что они лучше окупаются по LTV/CAC
- развивать повторные покупки, потому что repeat buyers дают 57.76% выручки
- раскатить test-вариант CTA после A/B-теста и мониторить метрики после релиза
- отдельно изучить mobile checkout, потому что mobile уступает desktop по конверсии
